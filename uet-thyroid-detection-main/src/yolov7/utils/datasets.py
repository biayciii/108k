...
img_formats = ['bmp', 'jpg', 'jpeg', 'png', 'tif', 'tiff', 'dng', 'webp', 'mpo', 'dcm']  # acceptable image suffixes
vid_formats = ['mov', 'avi', 'mp4', 'mpg', 'mpeg', 'm4v', 'wmv', 'mkv']  # acceptable video suffixes
BODY_CUT_THRESHOLD = 256
normalizer = ToTensor()
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            img0 = read_dcm_im(path)  # BGR
            # Remove irrelevant part under body
            img0 = remove_irrelevant_part(img0, BODY_CUT_THRESHOLD)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                im = read_dcm_im(im_file)
                im = remove_irrelevant_part(im, BODY_CUT_THRESHOLD)
                im = array2Image(im)
    ...
    def __getitem__(self, index):
        ...
        # Convert
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        # img = create_imbatch(img[0], 3).transpose(2, 0, 1)
        img = np.ascontiguousarray(img)
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        img = read_dcm_im(path) # BGR
        img = remove_irrelevant_part(img, BODY_CUT_THRESHOLD)
...
def extract_boxes(path='../coco/'):  # from utils.datasets import *; extract_boxes('../coco128')
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in img_formats:
            ...
            im = read_dcm_im(str(im_file))
            # Remove irrelevant parts and convert BGR to RGB
            im = remove_irrelevant_part(im, BODY_CUT_THRESHOLD)[..., ::-1]            
            h, w = im.shape[:2]
...
def read_dcm_im(im_file):
    im = pydicom.dcmread(im_file).pixel_array
    im = increase_count(im, 2)
    im = np.stack([im]*3, axis=-1)
    im = np.uint8(im)
    
    # 2 below code use to visual (comment both 2 line if not need)
    # im = normalizer(im)
    # im = np.asarray(to_pil_image(im*255))
    # End code for visual
    return im
...
def array2Image(im):
    # BGR to RGB then convert to PIL.Image
    im = Image.fromarray(np.uint8(im)[..., ::-1])
    im.format = 'dcm'
    return im

def remove_irrelevant_part(img: np.ndarray, threshold: int):
